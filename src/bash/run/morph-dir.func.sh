#!/bin/bash
# file: src/bash/run/morph-dir.func.sh
#------------------------------------------------------------------------------
# Purpose:
...
do_morph_dir(){
      ...
      # set -x
		...
		test -z ${to_srch:-} && {
         echo "You must export to_srch=<<str-to-search-for>> - it is empty !!!";
         exit 1
      }
		test -z ${to_repl:-} && {
         echo "You must export to_repl=<<str-to-replace-with>> - it is empty !!!";
         exit 1
      }
